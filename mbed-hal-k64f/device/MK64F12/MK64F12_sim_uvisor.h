...
/* Apparently, the Freescale MK64F12 SIM registers show a behavior which is
 * different from the documented one (see K64P144M120SF5RM, Rev. 2, January
 * 2014). SIM_SCGx registers can be written only in supervisor mode,
 * differently from what documented.
 *
 * Here all the macros for bitband access to these peripherals are defined. The
 * macros use an SVCall to handle the write to the desired bitbanded address.
 * The uvisor checks their legitimacy before actually writing to that address.
 *
 * The uvisor is present by default, but if it is not enabled, the write to the
 * register is performed as usual */
